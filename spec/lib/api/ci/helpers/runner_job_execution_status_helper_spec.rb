...
RSpec.describe API::Ci::Helpers::RunnerJobExecutionStatusHelper, feature_category: :runner_core do
  ...
  describe '#lazy_job_execution_status' do
    shared_examples 'legacy job execution status behavior' do |factory_name, build_association|
      ...
      context 'with only canceling builds' do
        ...
        it 'returns :active for runners with only canceling builds' do
          is_expected.to all(eq(:active))
        end
      end
    end
    ...
    [
      ...
      context "when #{description}" do
        ...
        context 'when FF `ci_read_job_execution_status_from_running_builds` is disabled' do
          ...
          it_behaves_like 'legacy job execution status behavior', factory_name, build_association
        end
      end
    end
  end
